...
def train(epoch, model, criterion, optimizer, train_loader, args, margin = 0.5):
    ...
    for i, data_ in enumerate(train_loader, 0):
        ...
        embed_feat, nonorm_feat = model(inputs)
        inter_ = 0
        dist_ap = 0
        dist_an = 0
        loss, inter_, dist_ap, dist_an = criterion(embed_feat, labels, nonorm_feat)
        ...
        if (i + 1) % freq == 0 or (i+1) == len(train_loader):
            print('Epoch: [{0:03d}][{1}/{2}]\t'
                  ...
                  'Pos {pos.avg:.4f}\t'
                  'Neg {neg.avg:.4f} \t'
                  'Margin {margin:.4f}'.format
                  (epoch + 1, i + 1, len(train_loader), batch_time=batch_time,
                   loss=losses, accuracy=accuracy, pos=pos_sims, neg=neg_sims,margin = margin))
